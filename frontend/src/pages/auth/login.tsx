    ...
    >
      ...
      {googleEnabled && (
        <a
          ...
          className="flex h-12 w-full items-center justify-center gap-3 border border-black/20 bg-white px-4 text-sm font-bold text-[#111] transition hover:border-black/45 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#3155ff]"
        >
          <GoogleMark />
          Google
        </a>
      ...
      {googleEnabled && (
        <div className="my-5 flex items-center gap-3" aria-hidden="true">
          <span className="h-px flex-1 bg-black/10" /><span className="text-[9px] font-bold uppercase tracking-[0.14em] text-black/35">or</span><span className="h-px flex-1 bg-black/10" />
        </div>
      )}

      <form onSubmit={handleLogin} className="space-y-4">
        <div>
          <label htmlFor="login-email" className="text-xs font-bold text-black/65">Email</label>
          <input id="login-email" type="email" required autoComplete="email" inputMode="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className={fieldClass} />
        </div>
        <div>
          <label htmlFor="login-password" className="text-xs font-bold text-black/65">Password</label>
          <div className="relative">
            <input id="login-password" type={showPassword ? "text" : "password"} required autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className={`${fieldClass} pr-14`} />
      ...
      {process.env.NODE_ENV !== "production" && (
        <details className="mt-5 border-t border-black/10">
          <summary className="cursor-pointer py-3 text-[9px] font-bold uppercase tracking-[0.14em] text-black/40">Local demo access</summary>
