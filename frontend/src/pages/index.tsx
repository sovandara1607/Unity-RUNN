...
import { formatEventDate, formatEventDateParts, formatEventTime, eventStatusLabel, eventStatusTone } from "../lib/eventFormat";
...
// The homepage previews a handful of races; /events is the full board.
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadHome() {
      const [me, eventsResult, statsResult] = await withMinSkeleton(() => Promise.all([
        ...
        api.listEvents({ limit: HOME_EVENT_LIMIT, statuses: publicEventStatuses }).then(
          (result) => ({ ok: true as const, result }),
          () => ({ ok: false as const, result: { events: [], total: 0 } }),
        ),
        api.getClubStats().then((result) => ({ ok: true as const, result }), () => ({ ok: false as const, result: null })),
      ]));
      setUser(me);
      setEvents(eventsResult.result.events);
      setTotalEvents(eventsResult.result.total);
      setEventsFailed(!eventsResult.ok);
      setStats(statsResult.result);
      setStatsFailed(!statsResult.ok);
      setLoading(false);
    }
    loadHome();
  }, [reloadKey]);
  ...
  return (
    <div className="min-h-screen text-white" style={{ backgroundColor: config.background_color }}>
      <PageMeta
        title=""
        description={`${config.hero_intro} ${config.mission_text}`.trim()}
      />
      ...
      <main>
        ...
        <section className="grid gap-0 overflow-hidden lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]" style={{ backgroundColor: acid }}>
          <div className="px-5 py-12 text-black sm:px-8 sm:py-16 lg:border-r lg:border-black/15">
            ...
            {/* Neither branch above fires when the request fails, so a third of this panel
                used to disappear with nothing to explain it. */}
            {!stats && !loading && statsFailed && (
              <div className="mt-10 flex flex-wrap items-center gap-4 border-t border-black/15 pt-6">
                <p className="text-xs font-bold uppercase tracking-[0.12em] text-black/60">Club numbers are unavailable right now.</p>
                <button onClick={retry} className="text-xs font-black uppercase tracking-[0.12em] underline underline-offset-4 transition hover:opacity-70">
                  Try again
                </button>
              </div>
            )}
          ...
          <div className="mx-auto max-w-[1440px]">
            ...
            {loading ? (
              ...
              <div className="topo-surface grid min-h-[360px] place-items-center border border-white/10 p-8 text-center">
                <div>
                  <CalendarDays className="mx-auto h-8 w-8 text-white/50" />
                  <h3 className="sport-display mt-5 text-4xl uppercase">The calendar did not load.</h3>
                  <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-white/60">This is a connection problem on our side, not an empty race calendar.</p>
                  <Button onClick={retry} className="mt-6">Try again</Button>
                </div>
              </div>
            ) : events.length === 0 ? (
              <div className="topo-surface grid min-h-[360px] place-items-center border border-white/10 p-8 text-center">
                <div><CalendarDays className="mx-auto h-8 w-8 text-white/50" /><h3 className="sport-display mt-5 text-4xl uppercase">The road is being mapped.</h3><p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-white/50">No public races are scheduled yet. Check back soon for the next start line.</p></div>
              </div>
            ) : selectedEvent ? (
              <div className="overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.035] lg:grid lg:min-h-[600px] lg:grid-cols-[minmax(330px,0.72fr)_minmax(0,1.28fr)]">
                <div className="border-b border-white/10 lg:border-b-0 lg:border-r">
                  <div className="flex items-center justify-between border-b border-white/10 px-5 py-4 sm:px-6">
                    <p className="font-mono text-[9px] font-black uppercase tracking-[0.18em] text-white/60">Upcoming races</p>
                    <p className="font-mono text-[9px] font-black text-white/50">
                      {totalEvents > events.length
                        ? `${String(events.length).padStart(2, "0")} of ${String(totalEvents).padStart(2, "0")}`
                        : `${String(events.length).padStart(2, "0")} races`}
                    </p>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <span className={`flex h-[74px] flex-col items-center justify-center border ${selected ? "border-black/20" : "border-white/15"}`}>
                            <span className={`font-mono text-[8px] font-black uppercase tracking-[0.15em] ${selected ? "text-black/50" : "text-white/55"}`}>{date.month}</span>
                            <span className="sport-display mt-1 text-4xl leading-none">{date.day}</span>
                            <span className={`mt-1 font-mono text-[8px] font-black uppercase ${selected ? "text-black/45" : "text-white/50"}`}>{date.weekday}</span>
                          </span>
                          <span className="min-w-0">
                            <span className={`font-mono text-[8px] font-black uppercase tracking-[0.15em] ${selected ? "text-black/45" : "text-white/55"}`}>
                            ...
                            <span className={`mt-3 flex items-center gap-1.5 truncate text-[10px] font-bold uppercase tracking-[0.08em] ${selected ? "text-black/55" : "text-white/60"}`}><MapPin className="h-3 w-3 shrink-0" />{event.location || "Location TBC"}</span>
                          </span>
                          <span className={`grid h-8 w-8 place-items-center rounded-full border transition ${selected ? "border-black bg-black" : "border-white/15 text-white/60 group-hover:border-white/40 group-hover:text-white"}`} style={selected ? { color: acid } : undefined}><ArrowRight className="h-3.5 w-3.5" /></span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <article key={selectedEvent.id} className="relative min-h-[560px] overflow-hidden lg:min-h-full" aria-live="polite">
                  ...
                  <div className="absolute inset-0 flex flex-col justify-between p-5 sm:p-8 lg:p-10">
                    <div className="flex items-start justify-between gap-4">
                      <StatusChip tone={eventStatusTone(selectedEvent.status)}>{eventStatusLabel(selectedEvent.status)}</StatusChip>
                      <span className="grid h-16 w-16 place-items-center rounded-full text-center text-black sm:h-20 sm:w-20" style={{ backgroundColor: acid }}><span><span className="sport-display block text-3xl leading-none sm:text-4xl">{formatEventDateParts(selectedEvent.event_date).day}</span><span className="font-mono text-[8px] font-black uppercase tracking-[0.12em]">{formatEventDateParts(selectedEvent.event_date).month}</span></span></span>
                    </div>

                    <div>
                      <div className="flex flex-wrap gap-2 text-[9px] font-black uppercase tracking-[0.1em] text-white/70">
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-black/25 px-3 py-2 backdrop-blur-sm"><CalendarDays className="h-3 w-3" />{formatEventDate(selectedEvent.event_date, "medium")}</span>
                      ...
                      <div className="mt-7 flex flex-wrap items-center gap-3">
                        <Button href={`/events/${selectedEvent.slug}`} className="hover:-translate-y-0.5">
