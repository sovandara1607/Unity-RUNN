...
export default function EventsPage() {
  ...
  return (
    <div className="min-h-screen text-white" style={{ backgroundColor: config.background_color }}>
      <PageMeta
        title="Race calendar"
        description={`Every upcoming ${config.club_name} race. Search by name, then filter by entry status, month or start location.`}
      />
          ...
          <>
            ...
            <div className="flex flex-col gap-3 border-y border-white/15 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full rounded-full border border-white/15 bg-white/5 py-2 pl-9 pr-3.5 text-sm text-white placeholder-white/40 outline-none transition focus:border-[var(--brand)]"
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4 border-b border-white/15 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <p className="flex shrink-0 items-center gap-2 font-mono text-[9px] font-black uppercase tracking-[0.16em] text-white/55"><SlidersHorizontal className="h-3.5 w-3.5" /> Refine the board</p>
                <div className="grid gap-2 sm:grid-cols-2">
                  <label className="relative">
                    ...
                    <CalendarDays className="pointer-events-none absolute right-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-white/55" />
                  </label>
                  <label className="relative">
                    ...
                    <MapPin className="pointer-events-none absolute right-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-white/55" />
                  </label>
                </div>
              </div>
              <div className="flex items-center justify-between gap-4 lg:justify-end">
                <p role="status" className="font-mono text-[9px] font-black uppercase tracking-[0.14em] text-white/55"><span className="text-white">{filtered.length}</span> {filtered.length === 1 ? "start" : "starts"} on the board</p>
            ...
            <div className="mt-4 flex min-h-[200px] flex-col justify-between gap-6 p-7 text-black sm:flex-row sm:items-center sm:p-10" style={{ backgroundColor: acid }}>
              ...
              <Link href="/about" className="inline-flex w-fit items-center gap-2 border-b border-black pb-1 text-xs font-bold uppercase tracking-[0.12em]">Meet the club <ArrowUpRight className="h-4 w-4" /></Link>
            </div>
          </>
        )}
      </main>
...
function EventCard({ event, primary }: { event: Event; primary: string }) {
  return (
    <Link href={`/events/${event.slug}`} className="group flex flex-col overflow-hidden border border-white/10 bg-[var(--surface-raised)] transition hover:border-white/30">
      <div className="relative h-56 w-full overflow-hidden bg-[var(--surface-muted)] sm:h-64">
        <EventArtwork coverImage={event.cover_image} eventName={event.name} variant="card" />
        <StatusChip tone={eventStatusTone(event.status)} className="absolute left-0 top-4">
      ...
      <div className="flex flex-1 flex-col justify-between p-5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-bold uppercase tracking-[0.06em] text-white/55">
          <span className="inline-flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" />{formatEventDate(event.event_date)}</span>
...
function ErrorState({ error, onRetry }: { error: string; onRetry: () => void }) {
  const { config } = useSiteConfig();
  return (
    <div className="topo-surface flex min-h-[320px] items-center justify-center border border-white/10 p-8 text-center">
      <div>
        <p className="sport-display text-4xl uppercase" style={{ color: config.primary_color }}>No signal.</p>
        <p className="mt-4 text-sm text-white/65">The race calendar could not load. This is a connection problem, not an empty calendar.</p>
