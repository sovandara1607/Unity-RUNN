...
// Service records audit log entries. Failures to write are logged but never propagated
type Service struct {
	repo writer
	log  *slog.Logger
}
...
// List returns audit entries most recent first, optionally filtered by entity_type, backed by a separate read repository
func (s *Service) List(ctx context.Context, r reader, filter ListFilter) ([]Entry, error) {
	return r.List(ctx, filter)
}

// Record writes an audit log entry. actorID is nil for system-initiated actions. entityID is nil when the action isn't tied to one entity.
